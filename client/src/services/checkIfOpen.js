function CheckIfOpen(venue, checkIns) {
  ...
  // Adjust times to account for overnight hours
  if (closeTime <= openTime) {
    closeTime.setUTCDate(closeTime.getUTCDate() + 1);
  }

  // If current time is past midnight but before today's opening time, adjust openTime to the previous day
  if (nowUtc < openTime && nowUtc.getUTCHours() < parseInt(openTimeParts[0])) {
